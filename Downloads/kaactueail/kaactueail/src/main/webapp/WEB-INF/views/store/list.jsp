				...
				<c:forEach var="cocklist" items="${list}">
					<div class="card" style="width: 18rem">
						<a class="move" href='/store/get?cNum=<c:out value="${cocklist.cNum}"/>'>
							<img src="${path}/resources/images/cockit/${cocklist.cName}.png"
					...
					<ul id="pageInfo" class="pageInfo" style="list-style: none;">
						...
						<c:forEach var="num" begin="${pageMaker.startPage}"
							...
							<li class="pageInfo_btn ${pageMaker.cri.pageNum == num ? "active":"" }">
								<a
								href="/store/list?pageNum=${num}&amount=${pageMaker.cri.amount}">${num}</a>
							</li>
						</c:forEach>
						...
						<c:if test="${pageMaker.next}">
							<li class="pageInfo_btn next"><a
								href="/store/list?pageNum=${pageMaker.endPage + 1 }&amount=${ pageMaker.cri.amount}">Next</a></li>
						</c:if>
					</ul>
				</div>
				<div class="btns_wrap"
